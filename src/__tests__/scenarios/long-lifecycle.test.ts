...
describe('LONG Position Lifecycle', () => {
  ...
  describe('Opening LONG Position', () => {
    it('should open a LONG position with planned data', async () => {
      ...
      const trade = await Trade.create({
        ...
        entryFee: 1,
        sumPlusFee: 1010, // Planned: 1000 + 1% fee
        amount: 0.01, // Planned: ~0.01 BTC
  ...
  describe('Filling LONG Position', () => {
    it('should fill LONG position with actual data from exchange', async () => {
      ...
      trade.sumPlusFee = 1012; // Actual: 1012 USD spent (with fee)
      trade.amount = 0.01008; // Actual: 0.01008 BTC received
      trade.filledDate = new Date();
      await trade.save();

      expect(trade.status).toBe(TradeStatus.FILLED);
      expect(trade.sumPlusFee).toBe(1012);
      expect(trade.amount).toBe(0.01008);
  ...
  describe('Closing LONG Position - Profitable', () => {
    ...
    it('should close LONG position with profit', async () => {
      ...
      const trade = await Trade.create({
        ...
        depositPercent: 10,
        entryFee: 1,
        sumPlusFee: 1010, // Spent 1010 USD
        amount: 0.01, // Got 0.01 BTC
        initialEntryPrice: 100000,
        initialAmount: 0.01,
        filledDate: new Date(),
      });
      ...
      expect(profitUSD).toBeCloseTo(79, 1);
    ...
    it('should handle large profits correctly', async () => {
      ...
      const trade = await Trade.create({
        ...
        depositPercent: 20,
        entryFee: 1,
        sumPlusFee: 2020, // Spent 2020 USD
        amount: 0.04, // Got 0.04 BTC at 50k
      ...
      // Exit value: 0.04 × 100,000 × 0.99 = 3960 USD
      // Profit: 3960 - 2020 = 1940 USD (~96% profit)
      expect(profitUSD).toBeCloseTo(1940, 1);
  ...
  describe('Closing LONG Position - Unprofitable', () => {
    it('should close LONG position with loss', async () => {
      ...
      expect(profitUSD).toBeCloseTo(-119, 1);
    ...
    it('should handle break-even trades (only lose fees)', async () => {
      ...
      // Loss: 990 - 1010 = -20 USD (lost to fees)
      expect(profitUSD).toBeCloseTo(-20, 1);
  ...
  describe('Profit Calculation Edge Cases', () => {
    ...
    it('should handle high exit fees correctly', async () => {
      ...
      // Exit value: 0.01 × 110,000 × 0.95 = 1045 USD
      // Profit: 1045 - 1010 = 35 USD (much less due to high fee)
      expect(profitUSD).toBeCloseTo(35, 1);
    });
  });
});
