...
describe('Portfolio Statistics', () => {
  ...
  describe('LONG Position Statistics', () => {
    ...
    it('should calculate total profit USD for all LONG trades', async () => {
      ...
      // Trade 3: +35 USD profit (high exit fee)
      await Trade.create({
        portfolioId: testPortfolioId,
        coinSymbol: 'ETH',
        tradeType: TradeType.LONG,
      ...
      // Total: 79 - 119 + 35 = -5 USD
      expect(totalProfitUSD).toBeCloseTo(-5, 1);
    });
    ...
    it('should calculate average profit USD for LONG trades', async () => {
      ...
      // Profits: 79 + 178 - 119 = 138 USD total
      // Average: 138 / 3 = 46 USD
      expect(avgProfitUSD).toBeCloseTo(46, 0);
    });
  ...
  describe('Best and Worst Trades', () => {
    it('should identify best and worst LONG trades by profit USD', async () => {
      const tradesData = [
        { exitPrice: 110000 }, // +79 USD
        { exitPrice: 90000 }, // -119 USD (worst)
        { exitPrice: 120000 }, // +178 USD (best)
        { exitPrice: 105000 }, // +39.5 USD
      ];
      ...
      expect(bestTrade.profit).toBeCloseTo(178, 0);

      expect(worstTrade.trade.exitPrice).toBe(90000);
      expect(worstTrade.profit).toBeCloseTo(-119, 0);
    });
  });
