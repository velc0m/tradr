...
export function SplitTradeModal({
  ...
  const handleAutoFill = (index: number) => {
    ...
    if (autoValue > 0) {
      const newAmounts = [...amounts];
      newAmounts[index] = autoValue.toFixed(10).replace(/\.?0+$/, '');
      setAmounts(newAmounts);
    }
  };
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="p-3 rounded-lg bg-muted/50 border">
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className={`font-medium ${Math.abs(remaining) < tolerance ? 'text-green-500' : 'text-red-500'}`}>
                ...
                {previews.map((preview, index) => {
                  ...
                  return (
                    <div key={index} className="grid grid-cols-4 gap-2 px-3 py-2 text-sm">
                      ...
                      <div className="text-right text-muted-foreground">{preview.percentage}%</div>
                    </div>
                  );
                })}
              </div>
