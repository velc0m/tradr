...
export interface ITrade extends Document {
    ...
    isAveragingShort?: boolean; // True if SHORT is used for averaging LONG position (internal operation)
...
export interface PortfolioStatistics {
    ...
    standardFees: number; // Fees from standard trades
    averagingFees: number; // Fees from averaging operations
    winRate: number;
    avgProfitUSD: number;
    avgProfitPercent: number;
    totalROI: number;
    ...
    // SHORT-specific metrics (standard SHORT only, excluding averaging)
    short: {
    ...
    // Averaging SHORT metrics (internal operations - excluded from USD statistics)
    averaging: {
        totalProfitCoins: Record<string, number>; // Coins earned through averaging
        totalProfitPercent: number;
        winRate: number;
        totalTrades: number;
    };
